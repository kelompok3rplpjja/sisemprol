            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="row">
                            <a style="margin-left: 20px" href="{{ url('/jadwal/create') }}"
                               class="btn btn-success pull-left"
                               title="Add New Jadwal">
                                <i class="fa fa-plus" aria-hidden="true"></i> Add New
                            </a>

                            <form method="GET" action="{{ url('/jadwal') }}" accept-charset="UTF-8"
                        ...
                        <div class="table-responsive">
                                ...
                                @foreach($jadwal as $item)
                                    <tr>
                                        ...
                                        <td>@if(isset($item->published_at)) {{ \Carbon\Carbon::parse($item->published_at)->toDayDateTimeString() }} @else
                                                N/A @endif</td>
                                        <td align="right">
                                            <a href="{{ url('/jadwal/' . $item->id) }}" title="View Jadwal">
                                                <button class="btn btn-info btn-sm"><i class="fa fa-eye"
                                                                                       aria-hidden="true"></i> View
                                                </button>
                                            </a>
                                            <a href="{{ url('/jadwal/' . $item->id . '/edit') }}" title="Edit Jadwal">
                                                <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                                                          aria-hidden="true"></i> Edit
                                                </button>
                                            </a>

                                            <form method="POST" action="{{ url('/jadwal' . '/' . $item->id) }}"
                            ...
                            <div class="pagination-wrapper"> {!! $jadwal->appends(['search' => Request::get('search')])->render() !!} </div>
                        </div>
